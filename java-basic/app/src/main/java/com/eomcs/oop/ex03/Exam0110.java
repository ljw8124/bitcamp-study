...
public class Exam0110 { // <- 패키지 멤버 클래스

  // 지금 당장 A 클래스 앞에 붙은 static은 고민하지 말라!
  // 이 예제의 목표는 인스턴스 변수이다.
  static class A { // => A 클래스는 Method Area에 생성되고 main을 사용할 때마다 쓸 뿐이다. => 중첩클래스
    // 중첩 클래스는 특정 영역에서만 사용할 수 있다.

    // 인스턴스 변수 => main 내에서만 사용, main이 호출될 때 마다 생성x
    // => new 명령을 통해 생성된다.

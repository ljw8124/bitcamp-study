...
public class Exam0130 {
  public static void main(String[] args) {
    ...
    b = 5; // OK!
    b = 6; // OK!
    // - 리터럴 5, 6은 4바이트 정수 값(int)이다.
    // - 정수 리터럴은 기본이 4바이트 크기이지만,
    // - byte 변수에 저장할 수 있다면 허락한다! ->비록 4바이트의 정수지만 byte의 값 내에서 허용되는 값이라면 가능 => 리터럴인 경우에만 허용된다
    ...
    // z = x + y; // 컴파일 오류! -> x와 y는 리터럴이 아니다. => z(변수)는 byte이고 x+y는 int 이기 때문에 그릇이 너무 작아서 오류
    // "자바의 정수 연산은 최소 단위가 4byte이다." -> 따라서 byte끼리 연산하였다고 하더라도 int로 되기 때문에 컴파일 오류
    // "그래서 byte나 short의 연산 단위가 기본으로 4byte이다."
    //
    // - 자바는 정수 변수에 대해 산술 연산을 수행할 때,
    //   그 변수의 값이 4바이트 보다 작다면(byte나 short 라면),
    //   4바이트로 만들어 연산을 수행한다. 자동으로 int로 변환
    ...
    // - 연산 결과도 당연히 4바이트이다.
    //   그래서 4바이트 값(int)을 1바이트 메모리(byte)에 넣지 못하기 때문에
    //   컴파일 오류가 발생하는 것이다! => 리터럴일 경우만 예외적으로 허용
    ...
    s3 = s1; // OK!
    s3 = s2; // OK!
    // s3 = s1 + s2; // 컴파일 오류! -> short지만 연산을 하면 int로 변환되기 때문에 short인 s3에 int를 넣을수 없다

...
public class Exam0910 {
  public static void main(String[] args) {
    ...
    f = i; // int(4) ==> float(4). 
    // 유효자릿수를 넘는 정수는 짤린다.
    // 주의! 컴파일 오류가 발생하지 않는다. 그냥 자기가 감당하는 범위까지만 출력될 뿐이다
    System.out.println(f);

    f = l; // long(8) ==> float(4)
    // 유효자릿수를 넘는 정수는 짤린다.
    // 주의! 컴파일 오류가 발생하지 않는다. 마찬가지로 자기가 감당하는 범위까지만 출력될 뿐이다
    System.out.println(f);
    ...
    // 주의! 컴파일 오류가 발생하지 않는다. -> 마찬가지로 자기가 감당하는 범위까지만 출력 될뿐이다
    System.out.println(d);       
  }
}
...
//   개발자들이 놓치는 경우가 많다! -> 오류는 나지않고 값만 이상하게 나오기 때문에